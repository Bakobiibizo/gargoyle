...
> **Scope:** 3 entity types (`metric`, `experiment`, `result`), 3 templates (`analytics-metric-tree`, `analytics-experiment-plan`, `analytics-anomaly-detection-investigation`), and every infrastructure component they touch: patch protocol, schema validation, optimistic locking, dedup pipeline, embedding search, claim grounding, status transitions, and relation integrity.
